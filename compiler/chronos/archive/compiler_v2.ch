// Chronos Self-Hosted Compiler v0.18
// With Struct Support (Feature 1: Struct Definition Parsing)
// New Features: struct definitions with fields
...
fn emit(cg: *Codegen, line: *i8) -> i64 {
    ...
    let i: i64 = 0;
    let max_line_len: i64 = 8192;  // SECURITY FIX v0.18: Prevent infinite loops
    let truncated: i64 = 0;
...
fn str_to_num(s: *i8) -> i64 {
    let result: i64 = 0;
    let i: i64 = 0;
    let max_digits: i64 = 19;  // SECURITY FIX v0.18: i64 max ~19 digits
    let digit_count: i64 = 0;
    let max_iterations: i64 = 100;  // SECURITY FIX v0.18: Prevent infinite loops
...
fn main() -> i64 {
    println("========================================");
    println("  CHRONOS COMPILER v0.19 (Phase 2)");
    println("  Variables + Field Access");
    println("========================================");
    println("");

    println("Phase 1: Reading source...");
    let source_addr: i64 = read_source_file("/tmp/test_phase2.ch");
    if (source_addr == 0) {
        println("❌ Read failed");
        return 1;
    }
    ...
    // Check if return has field access
    let return_result: [i64; 2];
    return_result[0] = -1;
    return_result[1] = -1;
    let return_number: i64 = 0;
    let is_field_return: i64 = parse_return_field(source, ret_pos, return_result);

    if (is_field_return >= 0) {
        print("✅ Return var ");
        print_int(return_result[0]);
        print(".field ");
        print_int(return_result[1]);
        println("");
    } else {
        ...
        let expr: *Expr = expr_addr;
        return_number = expr.left;  // Store the number
        print("✅ Return number: ");
        print_int(return_number);
        println("");
    }
    ...
    // Use field access codegen if we have assignments or field return
    if (g_assignment_count > 0 || is_field_return >= 0) {
        gen_program_with_fields(cg, return_result[0], return_result[1], return_number);
    } else {
        // Old path: expression only
        let ret_pos2: i64 = find_return(source);
        let expr_addr: i64 = parse_expression(source, ret_pos2);
        let expr: *Expr = expr_addr;
        gen_program(cg, expr);
    }
